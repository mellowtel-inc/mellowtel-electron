...
import { REFRESH_INTERVAL, MAX_DAILY_RATE as DEFAULT_MAX_DAILY_RATE } from "../constants";

export class RateLimiter {
  static MAX_DAILY_RATE: number = DEFAULT_MAX_DAILY_RATE;

  static async getRateLimitData(): Promise<{ timestamp: number; count: number }> {
  ...
  static async getLifetimeTotalCount(): Promise<{ lifetime_total_count: number }> {
  ...
  static async setHistoricData(initial_timestamp: number, lifetime_total_count: number): Promise<void> {
  ...
  static async resetRateLimitData(now: number, add_to_count: boolean = false): Promise<void> {
    await this.setRateLimitData(now, add_to_count ? 1 : 0);
  }

  static async getIfRateLimitReached(): Promise<boolean> {
    let mllwtl_rate_limit_object = await getLocalStorage("mllwtl_rate_limit_reached");
    if (
